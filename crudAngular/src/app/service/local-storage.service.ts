...
export class LocalStorageService {
  ...
  public login(name: any, pass: any) {
    ...
    if (localStorage.getItem('registro') != null) {
      ...
      console.log('si jala', loginData[0].nombre);

      for (let i = 0; i < loginData.length; i++) {
        loginData[i].nombre;
        loginData[i].pass;

        if (name === loginData[i].nombre && pass === loginData[i].pass) {
  ...
  functionFilter(
    value: any,
    arrayOriginal: Array<any>,
    arrayFilter: Array<any>
  ) {
    const dataFilter = arrayFilter.filter(
      (dato: any) =>
        dato.nombre.toLowerCase().includes(value.toLowerCase().trim()) ||
        dato.precio.toLowerCase().includes(value.toLowerCase().trim())
    );
    return (arrayOriginal = dataFilter);
  }
}
